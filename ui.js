(function () {
  ...
  const { lettering, dress } = OuroborosTypography;
  ...
  function render() {
    ...
    input.setAttribute("aria-label", mode === "decode" ? "Ciphertext" : "Plaintext");
    ...
    try {
      ...
      if (mode === "decode") {
        ...
        result.append(node("div", mode === "twin" ? twin : encrypted, "output literal"));
        ...
        if (mode === "twin") { const shared = node("p", "", "output"); shared.append(node("span", "Both encrypt to: "), node("span", encrypted, "literal")); result.append(shared); }
        else { const display = node("div", "", "chain literal"); for (const step of chain.slice(0, 150)) { const item = node("div", "", "step"); item.append(node("strong", step.plain), node("small", `+${step.shift}`), node("b", step.result)); display.append(item); } result.append(display); if (chain.length > 150) result.append(node("p", "Showing the first 150 chain steps; ciphertext above is complete.")); }
      }
